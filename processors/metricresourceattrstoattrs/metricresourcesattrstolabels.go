			...
			for k := 0; k < metrics.Len(); k++ {
				...
				if hasResourceServiceNameAttr || hasResourceServiceInstanceIDAttr {
					applyToMetricAttributes(metric, func(am pcommon.Map) {
						...
						_, amHasPeriodServiceInstanceId := am.Get(conventions.AttributeServiceInstanceID)
						_, amHasUnderscoreServiceInstanceId := am.Get(ocServiceInstanceIdAttrKey)
						if amHasPeriodServiceInstanceId && amHasUnderscoreServiceInstanceId {
							am.Remove(conventions.AttributeServiceInstanceID)
						}
					})
				}
			}
